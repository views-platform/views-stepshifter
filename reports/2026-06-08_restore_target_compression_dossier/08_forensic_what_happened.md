...
- **ShurfModel `log_target` bug** (`shurf_model.py:210-212`): the `log_target=True` branch assigns `pred_*` *before* `expm1`, so such a Shurf would emit log-space predictions. `fourtieth_symphony` is `log_target=True` → exposed (not in `big_chungus`). **This is a genuine, distinct, still-open bug.**
