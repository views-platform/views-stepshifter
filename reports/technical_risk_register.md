...
|---|---|
| **Tier** | 1 |
| **Trigger** | Running any stepshifter ensemble (`chunky_bunny` / `pink_ponyclub` / `skinny_love`) with `--train` (constituent retraining) **before confirming the per-model `envs/views_stepshifter` resolves to a `views-stepshifter` build that carries the fix** — i.e. before the fix is either released to a PyPI version satisfying the `>=1.0.0,<2.0.0` pin, or installed *editable* into that env from the fixed `main` checkout. Applies to **any** unreleased fix: the original `target_transform` work **and** the 2026-06-28 D-41 grid-id fix + Hurdle/Shurf extension. |
| **Source** | falsify (2026-06-09); generalized by deep register triage (2026-06-28) |
| **Status** | Open |
...
| **Notes** | The `EnsembleManager` trains each constituent by **subprocessing that model's `run.sh`**, which activates a **per-model conda env** that pip-installs the **published** `views-stepshifter` (1.x) — which has **no `target_transform` mechanism**. The config's `target_transform: "log1p"` is therefore **silently ignored**; the constituent trains on the **raw** target (the exact D-17/#114 bug), with no error signal. The entire `log1p` validation this session (`car_radio`/`bittersweet_symphony`/`counting_stars` cm MSLE ~0.41) was performed in the `views_pipeline` env (editable install of the fixed branch) — **a different env than the ensemble's execution path** — giving false confidence. Same observable failure as **D-17** (raw/mis-scaled predictions) but a **distinct root cause**: an *unreleased-fix + published-env execution path*, not frozen artifacts. The fix is not actually "applied" to an ensemble run until it is **released** (issue #55) or the per-model envs are provisioned with the fixed code. Cross-ref **D-17**, **D-14** (version/build drift), **D-20** (ensemble path unaudited), **D-29** (envs absent); issues #55 (release), #114 (config flip). **Update (2026-06-28) — generalized; the D-41 grid-id "stripe" fix is exposed to this SAME nullification, and it is acute for Angelica's imminent rerun.** The fix (plain `e10ec32`, Hurdle/Shurf extension `ff284a6`; PRs #96/#99) is on `main` but **unreleased to PyPI** (latest published 1.3.0 carries neither). So a `skinny_love` (PGM) / `pink_ponyclub` (CM) retrain will **silently re-bake the stripe** unless the per-model `envs/views_stepshifter` has `views-stepshifter` installed **editable**, pointing at the fixed `main` checkout (then `git pull` reaches it). A fresh or published install pulls **1.3.0** and the fix never applies — no error, false "it's fixed" confidence (the whole point of D-28). **Pre-run check:** `pip show views-stepshifter` inside `envs/views_stepshifter` must show the *editable* path to the fixed `main` checkout, not PyPI 1.3.0. **Robust mitigation:** cut a PyPI release (e.g. 1.4.0) carrying D-41 + the Hurdle extension so the pin pulls it. The lesson: the per-model-published-env path nullifies **any** unreleased views-stepshifter fix, not only `target_transform`. Cross-ref **D-41** (the fix being delivered). |
