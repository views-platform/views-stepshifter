...
class HurdleModel(StepshifterModel):
    ...
    **Step 2: Regression Stage**
    ...
    def _fit_by_step(self, step):
        ...
        positive_model.fit(self._target_pos, past_covariates=self._past_cov_pos)

        return (binary_model, positive_model)
    
    def _predict_by_step(self, step: int, sequence_number: int=0):
        # OVERRIDE: Help function for parallel execution
        binary_model, positive_model = self._models[step]
        pred_binary = self._predict(binary_model, step, sequence_number)
        pred_positive = self._predict(positive_model, step, sequence_number)
        return pred_binary.sort_index() * pred_positive.sort_index()
    ...
    def fit(self, df: pd.DataFrame):
        ...
        self._target_pos, self._past_cov_pos = zip(
            *[
                (t, p)
